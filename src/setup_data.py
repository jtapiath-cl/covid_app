...
def pull_data(data_folder: str, data_prod: str):
    ...
    log_loc = os.path.join(base_path, "logs", "setup-data.log")
    ...
    csv_fold = os.path.join("/app", data_prod)
    csv_file = os.path.join("/app", data_prod, "Covid-19_std.csv")
    data_src = os.path.join(git_file, "output", "producto1", "Covid-19_std.csv")
    data_fnl = os.path.join("/app", data_prod, "data.csv")
    ...
    del df_tmp
    try:
        date, timestamp = helpers.pull_last_update(json_f = json_file, field = "last_date")
